  ...
  return (
    <Provider store={store}>
      <Box display='flex' p={1}>
        <Button variant='contained' sx={{ margin: 1 }} onClick={() => props.decrement()}>-</Button>
        <Typography variant='h3'>{props.count}</Typography>
        <Button variant='contained' sx={{ margin: 1 }} onClick={() => props.increment()}>+</Button>
      </Box>
    </Provider>
  );
...
export function CounterWrapped() {
  return (
    <Provider store={store}>
      {React.createElement(connectedCounter)}
    </Provider>
  );
}
